...
abstract class MailboxSpec extends AkkaSpec {
  def name: String
  ...
  name should {
    ...
    @Test def `must create an unbounded mailbox`: Unit = {
      val config = UnboundedMailbox()
      val q = factory(config)
      ensureInitialMailboxState(config, q)
    }

    @Test def `must UnboundedMailbox.numberOfMessages must be consistent with queue size`: Unit = {
      ensureSingleConsumerEnqueueDequeue(UnboundedMailbox())
    }

    @Test def `must BoundedMailbox.numberOfMessages must be consistent with queue size`: Unit = {
      ensureSingleConsumerEnqueueDequeue(BoundedMailbox(1000, 10 milliseconds))
    }

    @Test def `must create a bounded mailbox with 10 capacity and with push timeout`: Unit = {
      ...
      assertThat(q.numberOfMessages, equalTo(config.capacity))
      ...
      assertThat(q.dequeue, equalTo(exampleMessage))
      assertThat(q.numberOfMessages, equalTo(config.capacity - 1))
      assertThat(q.hasMessages, equalTo(true))
    }

    @Test def `must dequeue what was enqueued properly for unbounded mailboxes`: Unit = {
      testEnqueueDequeue(UnboundedMailbox())
    }

    @Test def `must dequeue what was enqueued properly for bounded mailboxes`: Unit = {
      testEnqueueDequeue(BoundedMailbox(10000, -1 millisecond))
    }

    @Test def `must dequeue what was enqueued properly for bounded mailboxes with 0 pushTimeout`: Unit = {
      testEnqueueDequeue(BoundedMailbox(10, 0 millisecond), 20, 10, false)
    }

    @Test def `must dequeue what was enqueued properly for bounded mailboxes with pushTimeout`: Unit = {
  ...
  def ensureMailboxSize(q: MessageQueue, expected: Int): Unit = q.numberOfMessages match {
    case -1 | `expected` ⇒
      assertThat(q.hasMessages, equalTo((expected != 0)))
    case other ⇒
      assertThat(other, equalTo(expected))
  ...
  def ensureInitialMailboxState(config: MailboxType, q: MessageQueue) {
    ...
    q match {
      case aQueue: BlockingQueue[_] ⇒
        config match {
          assertThat(case BoundedMailbox(capacity, _) ⇒ aQueue.remainingCapacity, equalTo(capacity))
          case UnboundedMailbox()          ⇒ aQueue.remainingCapacity must be === Int.MaxValue
        }
      case _ ⇒
    }
    assertThat(q.numberOfMessages, equalTo(0))
  ...
  def testEnqueueDequeue(config: MailboxType,
    ...
    EventFilter.warning(pattern = ".*received dead letter from Actor.*MailboxSpec/deadLetters.*",
      occurrences = (enqueueN - dequeueN)) intercept {
        ...
        assertThat(ps.map(_.size).sum, equalTo(enqueueN //Must have produced 1000 messages))
        ...
        assertThat(cs.flatten.distinct.size, equalTo(dequeueN))
        //All consumed messages must have been produced
        assertThat((cs.flatten diff ps.flatten).size, equalTo(0))
        //The ones that were produced and not consumed
        assertThat((ps.flatten diff cs.flatten).size, equalTo((enqueueN - dequeueN)))
      }
  }
}
...
class CustomMailboxSpec extends AkkaSpec(CustomMailboxSpec.config) {
      @Test def `must support custom mailboxType`: Unit = {
      ...
      assertThat(queue.getClass, equalTo(classOf[CustomMailboxSpec.MyMailbox]))
    }
  }
}
...
class SingleConsumerOnlyMailboxVerificationSpec extends AkkaSpec(SingleConsumerOnlyMailboxVerificationSpec.mailboxConf) {
  ...
  "A SingleConsumerOnlyMailbox" should {
    @Test def `must support pathological ping-ponging`: Unit = within(30.seconds) {
      val total = 2000000
